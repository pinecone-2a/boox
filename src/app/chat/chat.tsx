        ...
        <div className="relative inline-block" ref={dropdownRef}>
          ...
          {isOpen && (
            <ul className="absolute right-0 mt-4 w-40 bg-white shadow-lg border rounded z-100 overflow-hidden">
              <li
                className="p-2 hover:bg-gray-200 cursor-pointer text-green-500  flex gap-2"
                onClick={() => {
                  openModal("ACCEPTED");
                }}
              >
              ...
              <li
                className="p-2 hover:bg-gray-200 cursor-pointer text-red-500  flex gap-2"
                onClick={() => {
                  openModal("REJECTED");
                }}
              >
          ...
          <Modal
            isOpen={isReviewModal.visible}
            onClose={() => {
              closeModal(isReviewModal.status);
            }}
            userId={receiver.id}
            reviewerId={sender.id}
            matchId={matchId}
            status={isReviewModal.status}
          />
        </div>
      </div>
          ...
          <div>
            {[...Array(5)].map((_, index: number) => (
              <div
                ...
                className={
                  index % 2 === 1
                    ? "flex self-end flex-row-reverse gap-2 items-center"
            ...
            <div
              ...
              className={
                msg.senderId === sender.id
                  ? "flex self-end flex-row-reverse gap-2"
                  : "flex items-center self-start gap-2"
              }
            >
              <Avatar>
                <AvatarImage
                  src={
                    msg.senderId === sender.id ? senderProfile : receiverProfile
                  }
                  alt="@shadcn"
                />
                <AvatarFallback></AvatarFallback>
              ...
              <p
                className={
                  msg.senderId === sender.id
                    ? "self-end bg-indigo-600 text-white pb-1 pl-2 pr-2 rounded-2xl "
                    : " pb-1 self-start pl-2 pr-2 rounded-2xl bg-secondary text-secondary-foreground"
                }
              >
                {msg.text}
              </p>
